<?php
// require('data/services.php');
// require('data/projects.php');
// require('data/Feature.php');




require('sections/header.php');
require('sections/banner.php');
require('sections/feature.php');
require('sections/about.php');
//require('sections/newsletter.php');
require('sections/service.php');
require('sections/video.php');
require('sections/media.php');
require('sections/project.php');
//require('sections/client.php');
require('sections/quote.php');
//require('sections/count.php');
require('sections/footer.php');


?>